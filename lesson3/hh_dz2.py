# 2) Доработать приложение таким образом, чтобы можно
# было искать разработчиков на разные языки
# программирования (Например Python, Java, C++)

# Я сначала ищу первые 5 страниц по вакансии программист,
# и потом из этих вакансий смотрю, есть необходимый язык разработки или нет
...
def request_to_hh():
    name_prog = input('Введите язык программирования для разработчика: ')
    all_page_req = []
    profession = 'Программист'
    session = requests.Session()
    for p in range(4):
        request = session.get('https://spb.hh.ru/search/vacancy', params={'text': profession, 'page': 1}, headers=headers)
    ...
    if all_page_req:
        ...
        for k in range(len(all_page_req)):
            if f_posit == 0:
                ...
                if name_prog in all_page_req[k]:
                    count_prof += 1
                    print(f'Ссылка на вакансию: {all_page_req[k-1]}')
    ...
    if count_prof == 0:
        print(f'По вашему запросу "{name_prog}" ничего не найдено')
